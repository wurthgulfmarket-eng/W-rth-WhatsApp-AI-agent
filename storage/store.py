"""
Lightweight SQLite storage - no external database service needed.
Tracks which company each WhatsApp number belongs to, and logs conversations.

Note: Render's free tier filesystem is ephemeral, so this database resets on
every deploy/restart - the dashboard will show live data between deploys but
not history across them. If you need persistent history, point this at a
hosted Postgres instead (see git history for storage/store.py before this
revert for a working psycopg2-based version) - swapping it back in requires
no changes to main.py/dashboard.py/ai/agent.py, since they only call the
...
# start/end below are "YYYY-MM-DD" strings (inclusive) - text comparison works
# correctly against ISO-formatted timestamps.
