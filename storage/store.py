"""
Postgres-backed storage (e.g. Supabase) - tracks which company each WhatsApp
number belongs to, and logs conversations. Required for persistence: Render's
free tier filesystem is ephemeral and wipes SQLite on every deploy/restart,
which is why the dashboard previously showed no historical data.
    ...
    if _pool is None:
        if not config.DATABASE_URL:
            raise RuntimeError(
                ...
                "(e.g. from Supabase) to store conversations and customer records."
            )
